...
/**
 * Reserva vertical = cabecera fija: logo + pastilla «Ahorro en electricidad» (mt-12 / sm:mt-14).
 * Mantener alineado con `AhorroLuzBrandHeader`.
 */
export const AHORRO_LUZ_HEADER_SPACER_CLASS =
  'h-[calc(max(0.875rem,env(safe-area-inset-top,0px))+8.125rem)] sm:h-[calc(max(1.125rem,env(safe-area-inset-top,0px))+8.875rem)]';

/** Padding superior del área scrollable bajo la misma cabecera fija. */
export const AHORRO_LUZ_SCROLL_TOP_PAD_CLASS =
  'pt-[calc(max(0.875rem,env(safe-area-inset-top,0px))+8.125rem)] sm:pt-[calc(max(1.125rem,env(safe-area-inset-top,0px))+8.875rem)]';
